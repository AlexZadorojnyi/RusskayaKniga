			...
			if(!reachedStart(thisClass)){
				if(!blockScroll){
					...
					$("." + thisClass + ".book:first").animate({right: "-=" + $("." + thisClass + ".book").outerWidth(true) + "px"}, scrollSpeed, function(){
						blockScroll = false;
						if(reachedStart(thisClass)) toggleButton(thisClass, "left", "off");
		...
		$(".scrollRight").click(function(){
			...
			if(!reachedEnd(thisClass)){
				if(!blockScroll){
					...
					$("." + thisClass + ".book:first").animate({right: "+=" + $("." + thisClass + ".book").outerWidth(true) + "px"}, scrollSpeed, function(){
						blockScroll = false;
						if(reachedEnd(thisClass)) toggleButton(thisClass, "right", "off");
		...
		*/
		function reachedEnd(thisClass) {
			...
			if(Math.round(bookOuterWidth * bookNum - containerWidth + bookPos - bookOuterWidth) >= 0){
				//$(".scrollRight."  + thisClass).animate({opacity: "1"});
				return false;
			} else {
				//$(".scrollRight."  + thisClass).animate({opacity: "0"});
				return true;
			}
		}
		...
		*/
		function reachedStart(thisClass) {
			...
			if(Math.round(bookPos + bookOuterWidth) <= 0){
				//toggleButton(thisClass, "left", "on");
				return false;
			} else {
				//toggleButton(thisClass, "left", "off");
				return true;
		...
		function toggleButton(thisClass, button, toggle){
			if(button == "right"){
				...
				if(toggle == "on"){
					$(".scrollLeft." + thisClass).animate({opacity: "1"});
					$(".scrollLeft." + thisClass).css("cursor", "pointer");
					//console.log(thisClass + " " + button + " " + toggle);
				} else if(toggle == "off"){
					$(".scrollLeft." + thisClass).animate({opacity: "0"});
