...
app.controller('myCtrl', ['$scope', function($scope) {
	...
	$scope.displayBookInfo = function(thisClass, book, lang){
		...
		if(lang == "en"){
			...
			var softCover = "Мягкий переплет";
		}
		
		//Title
		if(angular.isDefined(book.title)){
			newHTML = newHTML + "<span class='title'><strong>" + book.title + "</strong></span><br>";
		}
		
		//Author
		if(angular.isDefined(book.author)){
		...
		if(angular.isDefined(book.pages) && book.pages > 0) {
			...
			newHTML = newHTML + "<span class='pages'>" + book.pages + pages + "</span><br>";
		} else {
			newHTML = newHTML + "<br>";
		}
		...
		if(angular.isDefined(book.price)){
			if(angular.isDefined(book.sale) && book.sale > 0){
				newHTML = newHTML + "<span class='salePrice'>$" + parseFloat(Math.round((book.price - book.price * book.sale) * 100)/100).toFixed(2) + " </span><span class='regPrice'>$" + parseFloat(book.price).toFixed(2) + "</span>";
			} else {
				newHTML = newHTML + "<span class='price'>$" + parseFloat(book.price).toFixed(2) + "</span>";
			}
		}
		
		//Description
		if(angular.isDefined(book.desc)){
			newHTML = newHTML + "<p class='desc'>" + book.desc  + "</p><br>";
		}
